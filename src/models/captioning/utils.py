...
def compute_metrics(model, processor, test_dataloader, max_length):
    ...
    for batch in tqdm(test_dataloader):
        ...
        for label_group in labels:
            decoded_labels.append(
                processor.batch_decode(label_group, skip_special_tokens=True)
            )
    ...
    bleu1_score = bleu.compute(
        predictions=all_decoded_predictions, references=all_decoded_labels, max_order=1
    )
    bleu4_score = bleu.compute(
        predictions=all_decoded_predictions, references=all_decoded_labels
    )
...
def get_multiclassification_vit_tools():
    ...
    vit.load_state_dict(image_encoder_state_dict)

    return image_processor, vit
