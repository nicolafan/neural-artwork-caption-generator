...
def compute_metrics(eval_pred):
    ...
    multiclass_logits, multilabel_logits = list(predictions[:len(data.MULTICLASS_FEATURES)]), list(predictions[len(data.MULTICLASS_FEATURES):])
    multiclass_labels, multilabel_labels = list(labels[:len(data.MULTICLASS_FEATURES)]), list(labels[len(data.MULTICLASS_FEATURES):])
    ...
    for i, feature in enumerate(data.MULTICLASS_FEATURES):
        ...
        precision = precision_score(multiclass_labels[i], multiclass_preds[i], average="macro", zero_division=0)
        recall = recall_score(multiclass_labels[i], multiclass_preds[i], average="macro", zero_division=0)
        f1 = f1_score(multiclass_labels[i], multiclass_preds[i], average="macro", zero_division=0)
    ...
    for i, feature in enumerate(data.MULTILABEL_FEATURES):
        ...
        precision = precision_score(multilabel_labels[i], multilabel_preds[i], average="macro", zero_division=0)
        recall = recall_score(multilabel_labels[i], multilabel_preds[i], average="macro", zero_division=0)
        f1 = f1_score(multilabel_labels[i], multilabel_preds[i], average="macro", zero_division=0)
